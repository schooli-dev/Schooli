...
export const getTeacherModulesSchema = z.object({
  params: z.object({ teacherId: z.string().uuid() })
});

export const listTeacherAccessSchema = z.object({ query: z.object({}) });

export const replaceTeacherModulesSchema = z.object({
  params: z.object({ teacherId: z.string().uuid() }),
  body: z.object({ moduleIds: z.array(z.string().uuid()) })
});
